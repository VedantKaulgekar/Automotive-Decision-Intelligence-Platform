...
def generate_emission_dataset(N=7000, seed=42):
    ...
    machine_temperature = np.clip(35 + 60 * production_load + np.random.normal(0, 12, N), 30, 120)
    ...
    clean_signal = (
        ...
        + 0.025* cycle_time
        - 1.2  * power_factor
        + 5.0  * production_load * (machine_temperature / 120)
    )

    # noise=0.8 → CV≈91%, train≈93%, gap≈0.02 — excellent
    noisy_signal = clean_signal + np.random.normal(0, 0.8, N)
